#!/usr/bin/env python3
"""
Migrate fork-db/ from the old repo-name-organized layout to the new owner-organized layout.

Old layout:
  fork-db/
    ab/some-repo.json   ← keyed on first 2 chars of REPO NAME
                          each file has fork_families / orphaned_forks lists

New layout:
  fork-db/
    _metadata.json      ← format: owner-v1
    ce/celestiaorg.json ← keyed on first 2 chars of OWNER NAME
                          each file has a flat repos list

Usage:
  python3 migrate_db.py fork-db/ fork-db-new/
  python3 migrate_db.py fork-db/ fork-db-new/ --verify
"""
...
def migrate(source_dir: str, dest_dir: str, verify: bool = False) -> int:
    ...
    print(f"  python3 build_index.py")
    return 0
